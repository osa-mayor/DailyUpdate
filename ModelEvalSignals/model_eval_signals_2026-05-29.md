# Model & Eval Signals (2026-05-29)

## 오늘의 요약
멀티모달 모델의 다양화와 소프트웨어 엔지니어링 에이전트 역량 평가를 위한 새로운 벤치마크 및 학습 환경이 주목받고 있으며, 코딩 성능 중심의 벤치마크 결과가 업데이트되었습니다.

### 오늘의 핵심 포인트
- ByteDance 및 NVIDIA 등에서 새로운 멀티모달(any-to-any, object localization) 모델 공개
- SWE-bench 팀의 CodeClash 출시 및 mini-SWE-agent의 성과 등 에이전트 중심의 평가/학습 도구 진화
- LiveCodeBench를 통한 최신 모델(Gemini 시리즈 등)의 코딩 성능 스냅샷 업데이트

**오늘의 태그**: Multimodal, AI Agents, LLM Benchmarking, Software Engineering
...
- any-to-any 파이프라인 태그를 가진 모델입니다.
- Hugging Face에서 2,500회 이상의 다운로드와 952개의 좋아요를 기록하며 트렌딩 중입니다.

**태그**: ByteDance, Lance, any-to-any

**Metrics**: {"likes": 952, "downloads": 2506, "num_parameters": 0, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=952, downloads=2506, pipeline_tag=any-to-any

---

## 2. [Jackrong/Qwopus3.6-27B-v2-GGUF](https://huggingface.co/Jackrong/Qwopus3.6-27B-v2-GGUF)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Jackrong/Qwopus3.6-27B-v2-GGUF 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 27B 규모의 모델입니다.

### 핵심 포인트
- Jackrong/Qwopus3.6-27B-v2-GGUF 모델의 높은 관심도(downloads 24,336회) 확인
- 27B 규모의 파라미터를 가진 모델로, GGUF 포맷으로 제공됨
- image-text-to-text 기능을 지원하는 멀티모달 성격의 모델

**태그**: Jackrong/Qwopus3.6-27B-v2-GGUF, GGUF, image-text-to-text

**Metrics**: {"likes": 169, "downloads": 24336, "num_parameters": 27320697856, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=169, downloads=24336, pipeline_tag=image-text-to-text
...
### 요약
NVIDIA에서 공개한 3B 규모의 image-text-to-text 모델인 nvidia/LocateAnything-3B가 Hugging Face에서 주목받고 있습니다. 해당 모델은 이미지 내 객체 위치 파악 등 시각-언어 작업에 특화된 것으로 보입니다.

### 핵심 포인트
- NVIDIA에서 출시한 3B 파라미터 규모의 모델
- image-text-to-text 파이프라인을 지원하는 멀티모달 모델
- 최근 1,755회의 다운로드와 162개의 likes를 기록하며 트렌딩 중

**태그**: NVIDIA, LocateAnything-3B, Multimodal

**Metrics**: {"likes": 162, "downloads": 1755, "num_parameters": 3830665968, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=162, downloads=1755, pipeline_tag=image-text-to-text
