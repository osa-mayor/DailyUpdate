# Model & Eval Signals (2026-06-23)

## 오늘의 요약
Hugging Face를 중심으로 한 신규 모델들의 높은 관심과 더불어, 소프트웨어 엔지니어링 에이전트의 역량을 평가하고 학습시키기 위한 새로운 벤치마크 및 도구들이 등장하며 에이전트 중심의 평가 체계가 고도화되고 있습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 GLM-5.2, MiniMax-M3, VibeThinker-3B 등 다양한 규모와 멀티모달 기능을 갖춘 모델들이 높은 다운로드 수를 기록하며 트렌드를 주도함
- SWE-bench 팀에서 목표 지향적 개발 역량 평가를 위한 CodeClash와 에이전트 학습 도구인 SWE-smith를 공개하며 소프트웨어 엔지니어링 에이전트 분야의 기술적 진보를 보여줌
- LiveCodeBench를 통해 O4-Mini 및 Gemini 시리즈 모델들의 코딩 성능 스냅샷이 업데이트되며 모델 간 경쟁이 지속됨

**오늘의 태그**: LLM, Multimodal, Software Engineering Agents, Benchmark, Hugging Face

## 1. [zai-org/GLM-5.2](https://huggingface.co/zai-org/GLM-5.2)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Hugging Face에서 높은 관심을 받고 있는 zai-org/GLM-5.2 모델에 대한 동향입니다. 해당 모델은 text-generation 태그를 가진 대규모 언어 모델로, 높은 다운로드 수를 기록하며 주목받고 있습니다.

### 핵심 포인트
- zai-org/GLM-5.2 모델이 33,589회의 다운로드와 1,994개의 likes를 기록하며 트렌딩 중입니다.
- 모델의 파라미터 규모는 약 753B(753,329,940,480)로 매우 큰 규모를 가집니다.
- 주요 태스크는 text-generation 파이프라인을 지원합니다.

**태그**: GLM-5.2, text-generation, LLM

**Metrics**: {"likes": 1994, "downloads": 33589, "num_parameters": 753329940480, "pipeline_tag": "text-generation"}

### 원문 설명
likes=1994, downloads=33589, pipeline_tag=text-generation
...
### 요약
MiniMaxAI에서 공개한 MiniMax-M3 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 이미지와 텍스트를 동시에 처리하는 멀티모달 기능을 제공합니다.

### 핵심 포인트
- MiniMax-M3 모델의 Hugging Face 다운로드 수가 119,967회를 기록하며 높은 관심을 받고 있음
- pipeline_tag는 image-text-to-text로, 시각 정보와 텍스트를 결합한 작업에 특화됨
- 모델 파라미터 규모는 약 427B로 확인됨

**태그**: MiniMax-M3, Multimodal, Hugging Face

**Metrics**: {"likes": 1203, "downloads": 119967, "num_parameters": 427040140160, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1203, downloads=119967, pipeline_tag=image-text-to-text
...
### 요약
WeiboAI에서 공개한 VibeThinker-3B 모델이 Hugging Face에서 주목받고 있습니다. 약 3B 파라미터 규모의 text-generation 모델로, 최근 높은 다운로드 수를 기록하며 트렌드에 진입했습니다.

### 핵심 포인트
- WeiboAI에서 공개한 3B 규모의 text-generation 모델
- 32,385회의 다운로드와 603개의 likes를 기록하며 높은 관심을 받음
- 약 3.09B(3,085,938,688) 파라미터를 보유한 모델

**태그**: WeiboAI, VibeThinker-3B, text-generation

**Metrics**: {"likes": 603, "downloads": 32385, "num_parameters": 3085938688, "pipeline_tag": "text-generation"}

### 원문 설명
likes=603, downloads=32385, pipeline_tag=text-generation
