# Model & Eval Signals (2026-06-04)

## 오늘의 요약
Hugging Face의 신규 멀티모달 및 텍스트 생성 모델 공개와 더불어, SWE-bench 및 LiveCodeBench를 통한 소프트웨어 엔지니어링 및 코딩 역량 평가 중심의 벤치마크 업데이트가 활발히 진행되었습니다.

### 오늘의 핵심 포인트
- LiquidAI, PaddlePaddle, Stepfun-ai 등에서 새로운 텍스트 및 멀티모달 모델 공개
- SWE-bench의 새로운 평가 지표(CodeClash) 및 에이전트 학습 환경(SWE-smith) 도입
- LiveCodeBench를 통한 최신 모델(O4-Mini, Gemini 시리즈)의 코딩 성능 지표 업데이트

**오늘의 태그**: LLM, Multimodal, Software Engineering, Benchmark, Coding
...
### 핵심 포인트
- LiquidAI의 LFM2.5-8B-A1B 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 트렌딩 중입니다.
- 모델의 파라미터 규모는 약 8.47B입니다.
- 주요 태그는 text-generation이며, 6만 회 이상의 다운로드를 기록했습니다.

**태그**: LiquidAI, LFM2.5-8B-A1B, text-generation

**Metrics**: {"likes": 472, "downloads": 60171, "num_parameters": 8467856832, "pipeline_tag": "text-generation"}

### 원문 설명
likes=472, downloads=60171, pipeline_tag=text-generation

---

## 2. [PaddlePaddle/PaddleOCR-VL-1.6](https://huggingface.co/PaddlePaddle/PaddleOCR-VL-1.6)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
PaddlePaddle에서 공개한 PaddleOCR-VL-1.6 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 멀티모달 모델입니다.

### 핵심 포인트
- PaddleOCR-VL-1.6 모델의 Hugging Face 트렌딩 기록
- image-text-to-text 파이프라인 태그를 가진 멀티모달 모델
- 약 9.59억 개의 파라미터를 보유한 모델 규모

**태그**: PaddleOCR-VL-1.6, image-text-to-text, multimodal

**Metrics**: {"likes": 210, "downloads": 4829, "num_parameters": 958588736, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=210, downloads=4829, pipeline_tag=image-text-to-text
...
**Metrics**: {"likes": 228, "downloads": 17965, "num_parameters": 201365316160, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=228, downloads=17965, pipeline_tag=image-text-to-text
