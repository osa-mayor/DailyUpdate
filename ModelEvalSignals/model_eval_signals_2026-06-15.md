# Model & Eval Signals (2026-06-15)

## 오늘의 요약
Hugging Face에서는 대규모 멀티모달 모델들의 높은 관심을 확인할 수 있으며, SWE-bench와 LiveCodeBench를 중심으로 소프트웨어 엔지니어링 및 코딩 역량 평가를 위한 새로운 벤치마크와 에이전트 학습 도구들이 활발히 논의되고 있습니다.

### 오늘의 핵심 포인트
- Hugging Face 내 대규모 파라미터(400B급) 및 멀티모달(Image-to-Text) 모델들의 높은 다운로드 수 기록
- 단순 작업 수행을 넘어 목표 지향적 개발 역량을 측정하는 CodeClash 등 새로운 소프트웨어 엔지니어링 벤치마크 등장
- LiveCodeBench를 통한 최신 모델(Gemini, O4-Mini 등)의 코딩 성능 실시간 모니터링 및 비교

**오늘의 태그**: LLM, Multimodal, Software Engineering Agent, Benchmark, Coding Evaluation

## 1. [prefeitura-rio/Rio-3.5-Open-397B](https://huggingface.co/prefeitura-rio/Rio-3.5-Open-397B)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
prefeitura-rio/Rio-3.5-Open-397B 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 약 403B 파라미터를 가진 대규모 모델로, image-text-to-text 파이프라인을 지원합니다.

### 핵심 포인트
- prefeitura-rio/Rio-3.5-Open-397B 모델의 높은 관심도(downloads 112,371회) 확인
- 약 403B 규모의 대형 파라미터를 보유한 모델
- image-text-to-text 태그를 통한 멀티모달 기능 제공

**태그**: prefeitura-rio/Rio-3.5-Open-397B, image-text-to-text, Large Language Model

**Metrics**: {"likes": 259, "downloads": 112371, "num_parameters": 403397928944, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=259, downloads=112371, pipeline_tag=image-text-to-text
...
### 핵심 포인트
- MiniMax-M3 모델의 Hugging Face 내 높은 관심도 반영
- image-text-to-text 파이프라인을 지원하는 멀티모달 모델
- 약 427B 규모의 파라미터를 보유한 대형 모델

**태그**: MiniMax-M3, Multi-modal, Hugging Face

**Metrics**: {"likes": 477, "downloads": 6643, "num_parameters": 427040140160, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=477, downloads=6643, pipeline_tag=image-text-to-text

---

## 3. [unsloth/diffusiongemma-26B-A4B-it-GGUF](https://huggingface.co/unsloth/diffusiongemma-26B-A4B-it-GGUF)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending
...
### 핵심 포인트
- unsloth/diffusiongemma-26B-A4B-it-GGUF 모델의 높은 관심도(downloads 80,118회)
- image-text-to-text 작업을 위한 파이프라인 지원
- 약 25.25B 파라미터를 보유한 모델 규모

**태그**: unsloth, diffusiongemma, GGUF

**Metrics**: {"likes": 257, "downloads": 80118, "num_parameters": 25250987068, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=257, downloads=80118, pipeline_tag=image-text-to-text
