# Model & Eval Signals (2026-06-07)

## 오늘의 요약
Hugging Face에서는 LiquidAI와 Google의 새로운 모델들이 주목받고 있으며, SWE-bench와 LiveCodeBench를 중심으로 소프트웨어 엔지니어링 및 코딩 역량 평가를 위한 새로운 벤치마크와 모델 성과가 활발히 공유되었습니다.

### 오늘의 핵심 포인트
- LiquidAI 및 Google의 신규 모델(LFM2.5, gemma-4)이 Hugging Face에서 높은 관심을 받으며 텍스트 및 멀티모달 성능을 입증함
- SWE-bench 팀에서 목표 지향적 개발 역량 측정을 위한 새로운 벤치마크 'CodeClash'와 에이전트 학습 환경 'SWE-smith'를 공개함
- LiveCodeBench를 통해 O4-Mini 및 Gemini 시리즈 등 최신 모델들의 코딩 성능(avg_pass@1) 경쟁이 지속됨

**오늘의 태그**: LLM, Benchmark, Software Engineering Agents, Coding Evaluation, Hugging Face
...
### 핵심 포인트
- LiquidAI의 LFM2.5-8B-A1B 모델이 높은 다운로드 수(95,440회)를 기록하며 트렌딩 중입니다.
- 모델의 파라미터 규모는 약 8.47B입니다.
- text-generation 파이프라인을 지원하는 모델입니다.

**태그**: LiquidAI, LFM2.5-8B-A1B, text-generation

**Metrics**: {"likes": 530, "downloads": 95440, "num_parameters": 8467856832, "pipeline_tag": "text-generation"}

### 원문 설명
likes=530, downloads=95440, pipeline_tag=text-generation
...
### 요약
Unsloth에서 공개한 gemma-4-12b-it-GGUF 모델이 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 이미지와 텍스트를 동시에 처리할 수 있는 멀티모달 기능을 제공합니다.

### 핵심 포인트
- unsloth/gemma-4-12b-it-GGUF 모델의 높은 다운로드 수(458,174회) 기록
- image-text-to-text 파이프라인을 지원하는 멀티모달 모델
- 약 11.9B 파라미터를 보유한 GGUF 포맷 모델

**태그**: unsloth, gemma-4, GGUF

**Metrics**: {"likes": 412, "downloads": 458174, "num_parameters": 11907350576, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=412, downloads=458174, pipeline_tag=image-text-to-text
...
### 핵심 포인트
- google/gemma-4-12B 모델의 높은 관심도 반영 (downloads: 84,549)
- 약 11.96B 규모의 파라미터를 가진 모델
- any-to-any 파이프라인 태그를 통한 범용적 활용 가능성

**태그**: google, gemma-4-12B, any-to-any

**Metrics**: {"likes": 369, "downloads": 84549, "num_parameters": 11959730224, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=369, downloads=84549, pipeline_tag=any-to-any
