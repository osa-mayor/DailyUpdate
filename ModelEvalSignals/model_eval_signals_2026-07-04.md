# Model & Eval Signals (2026-07-04)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 및 코딩 역량 평가를 위한 새로운 벤치마크/에이전트 도구들의 업데이트가 주를 이루었습니다.

### 오늘의 핵심 포인트
- Baidu, NVIDIA 등 주요 기업의 특화 모델(OCR, Text-generation)이 Hugging Face에서 높은 관심을 받음
- SWE-bench를 중심으로 목표 지향적 개발 역량 평가(CodeClash) 및 에이전트 학습 도구(SWE-smith) 등 고도화된 평가 환경 등장
- LiveCodeBench를 통해 Gemini 및 O4-Mini 등 최신 모델들의 코딩 성능 스냅샷 확인

**오늘의 태그**: LLM, Benchmark, AI Agents, Coding, HuggingFace
...
- 88만 회 이상의 높은 다운로드 수와 1,680개의 likes 확보
- image-text-to-text 파이프라인을 활용한 OCR 작업 수행

**태그**: Baidu, Unlimited-OCR, OCR

**Metrics**: {"likes": 1680, "downloads": 885040, "num_parameters": 3336106240, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1680, downloads=885040, pipeline_tag=image-text-to-text
...
### 요약
Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있는 zai-org/GLM-5.2 모델입니다. 약 753B 규모의 파라미터를 가진 text-generation 태스크용 모델입니다.

### 핵심 포인트
- zai-org/GLM-5.2 모델의 높은 관심도 반영 (downloads 191,462회)
- 약 753.3B 규모의 대형 파라미터를 보유한 모델
- text-generation 파이프라인을 지원하는 모델 트렌드

**태그**: GLM-5.2, text-generation, Large Language Model

**Metrics**: {"likes": 3328, "downloads": 191462, "num_parameters": 753329940480, "pipeline_tag": "text-generation"}

### 원문 설명
likes=3328, downloads=191462, pipeline_tag=text-generation
...
### 요약
NVIDIA에서 공개한 Qwen3.6-27B-NVFP4 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 text-generation 태스크를 위한 모델로 분류됩니다.

### 핵심 포인트
- nvidia/Qwen3.6-27B-NVFP4 모델의 높은 관심도 반영 (downloads: 94,465)
- 약 18.16B 파라미터를 보유한 text-generation 모델
- Hugging Face 내 모델 트렌딩 지표 확인

**태그**: NVIDIA, Qwen3.6-27B-NVFP4, text-generation

**Metrics**: {"likes": 227, "downloads": 94465, "num_parameters": 18164649200, "pipeline_tag": "text-generation"}

### 원문 설명
likes=227, downloads=94465, pipeline_tag=text-generation
