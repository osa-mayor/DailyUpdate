# Model & Eval Signals (2026-08-29)

## 오늘의 요약
Hugging Face에서는 GLM-5.3 및 Qwen 기반의 다양한 특화 모델(GGUF, MLX)이 주목받고 있으며, LiveCodeBench에서는 O4-Mini와 Gemini 시리즈 모델들이 상위권 성적을 기록하며 코딩 성능 경쟁을 보여주었습니다.

### 오늘의 핵심 포인트
- Hugging Face 내 GLM-5.3 및 Qwen 기반 모델들의 높은 관심과 다운로드 기록
- GGUF 및 MLX 포맷을 활용한 다양한 파이프라인(image-text-to-text 등) 지원 모델 등장
...
### 요약
zai-org에서 공개한 GLM-5.3 모델이 Hugging Face에서 주목받고 있습니다. 해당 모델은 text-generation 파이프라인을 지원하는 대규모 언어 모델입니다.

### 핵심 포인트
- zai-org에서 공개한 GLM-5.3 모델의 등장
- 1,043개의 likes를 기록하며 모델 트렌드 형성
- text-generation 태그를 사용하는 대규모 파라미터 모델

**태그**: GLM-5.3, text-generation, LLM

**Metrics**: {"likes": 1043, "downloads": 0, "num_parameters": 753329940480, "pipeline_tag": "text-generation"}

### 원문 설명
likes=1043, downloads=0, pipeline_tag=text-generation
...
- 4,354회의 다운로드와 503개의 likes를 기록하며 높은 관심을 받음
- image-text-to-text 파이프라인을 지원하는 멀티모달 성격의 모델

**태그**: unsloth, Qwen3.8-Flash-Next-GGUF, image-text-to-text

**Metrics**: {"likes": 503, "downloads": 4354, "num_parameters": 176943899520, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=503, downloads=4354, pipeline_tag=image-text-to-text
...
### 핵심 포인트
- 83,352회의 높은 downloads를 기록하며 트렌드에 진입함
- image-text-to-text 파이프라인 태그를 가진 멀티모달 성격의 모델임
- 약 4.67B 규모의 파라미터를 보유함

**태그**: Qwen3.8-27B-Uncensored-MLX, image-text-to-text, MLX

**Metrics**: {"likes": 1180, "downloads": 83352, "num_parameters": 4665462000, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1180, downloads=83352, pipeline_tag=image-text-to-text
