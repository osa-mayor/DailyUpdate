# Model & Eval Signals (2026-05-08)

## 오늘의 요약
Hugging Face의 신규 트렌딩 모델(ZAYA1-8B, DeepSeek-V4-Pro 등) 등장과 함께, 소프트웨어 엔지니어링 에이전트 역량 및 목표 지향적 개발 능력을 측정하기 위한 새로운 벤치마크(CodeClash, SWE-smith)와 평가 리포트(METR)가 주요 흐름을 형성하고 있습니다.

### 오늘의 핵심 포인트
- Hugging Face 내 고성능/특화 모델(Text-gen, Image-to-video)의 트렌딩 진입
- 단순 작업 수행을 넘어 목표 지향적 개발 역량을 평가하는 새로운 벤치마크(CodeClash) 및 학습 환경(SWE-smith) 등장
- METR를 통한 주요 모델(GPT-5, DeepSeek, Qwen)에 대한 심층 평가 리포트 발표

**오늘의 태그**: LLM_Trending, Software_Engineering_Agents, Benchmark_Update, Evaluation_Report

## 1. [Zyphra/ZAYA1-8B](https://huggingface.co/Zyphra/ZAYA1-8B)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Zyphra에서 공개한 ZAYA1-8B 모델이 Hugging Face에서 주목받고 있습니다. 약 8.8B 파라미터를 가진 이 모델은 최근 다운로드와 좋아요 수가 상승하며 트렌딩 모델로 진입했습니다.

### 핵심 포인트
- Zyphra의 ZAYA1-8B 모델이 Hugging Face에서 트렌딩 중
- 모델 파라미터 규모는 약 8.8B 수준
- likes 179회, downloads 539회를 기록하며 사용자 관심을 확보

**태그**: Zyphra, ZAYA1-8B, Model Trending

**Metrics**: {"likes": 179, "downloads": 539, "num_parameters": 8840488784, "pipeline_tag": ""}

### 원문 설명
likes=179, downloads=539, pipeline_tag=

---

## 2. [TenStrip/LTX2.3-10Eros](https://huggingface.co/TenStrip/LTX2.3-10Eros)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
TenStrip/LTX2.3-10Eros 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 image-to-video 파이프라인을 지원하는 것이 특징입니다.

### 핵심 포인트
- TenStrip/LTX2.3-10Eros 모델의 Hugging Face 트렌딩 기록
- image-to-video 파이프라인 태그를 보유한 모델
- 28,215회의 다운로드와 149개의 likes를 기록 중

**태그**: TenStrip/LTX2.3-10Eros, image-to-video, Hugging Face

**Metrics**: {"likes": 149, "downloads": 28215, "num_parameters": 0, "pipeline_tag": "image-to-video"}

### 원문 설명
likes=149, downloads=28215, pipeline_tag=image-to-video
...
### 요약
DeepSeek-AI에서 공개한 DeepSeek-V4-Pro 모델이 Hugging Face에서 높은 관심을 받고 있습니다. 약 8,616억 개의 파라미터를 보유한 text-generation 모델로, 높은 다운로드 수를 기록하며 트렌드에 진입했습니다.

### 핵심 포인트
- DeepSeek-V4-Pro 모델의 Hugging Face 트렌드 급상승
- 약 861.6B 규모의 파라미터를 가진 대규모 text-generation 모델
- 94만 회 이상의 높은 다운로드 수와 3,713개의 likes 기록

**태그**: DeepSeek-V4-Pro, text-generation, Large Language Model

**Metrics**: {"likes": 3713, "downloads": 946264, "num_parameters": 861608274846, "pipeline_tag": "text-generation"}

### 원문 설명
likes=3713, downloads=946264, pipeline_tag=text-generation
