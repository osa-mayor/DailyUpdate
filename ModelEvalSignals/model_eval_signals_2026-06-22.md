# Model & Eval Signals (2026-06-22)

## 오늘의 요약
Hugging Face의 다양한 규모별 신규 모델(3B~753B)이 트렌딩되는 가운데, SWE-bench와 LiveCodeBench를 중심으로 소프트웨어 엔지니어링 및 코딩 역량 평가와 에이전트 학습 도구에 대한 논의가 활발히 진행되었습니다.

### 오늘의 핵심 포인트
- Hugging Face 내 다양한 파라미터 규모(3B, 12B, 753B) 및 양자화(GGUF) 모델들의 높은 관심과 다운로드 기록
- SWE-bench의 새로운 벤치마크 'CodeClash' 공개 및 'mini-SWE-agent'의 성과 등 목표 지향적 개발 에이전트 평가 기술 발전
- LiveCodeBench를 통한 Gemini 및 O4-Mini 등 주요 모델들의 코딩 성능(pass@1) 비교 및 벤치마크 스냅샷 업데이트
...
### 핵심 포인트
- WeiboAI에서 출시한 3B 규모의 text-generation 모델입니다.
- 20,277회의 다운로드와 551개의 likes를 기록하며 높은 관심을 받고 있습니다.
- 모델 파라미터 수는 약 3.08B입니다.

**태그**: WeiboAI, VibeThinker-3B, text-generation

**Metrics**: {"likes": 551, "downloads": 20277, "num_parameters": 3085938688, "pipeline_tag": "text-generation"}

### 원문 설명
likes=551, downloads=20277, pipeline_tag=text-generation
...
### 핵심 포인트
- 모델명: yuxinlu1/gemma-4-12B-agentic-fable5-composer2.5-v2-3.5x-tau2-GGUF
- 21,730회의 높은 다운로드 수와 260개의 likes를 기록하며 주목받고 있음
- 약 11.9B 파라미터를 가진 text-generation 목적의 모델

**태그**: Gemma-4, GGUF, Text-Generation

**Metrics**: {"likes": 260, "downloads": 21730, "num_parameters": 11907350576, "pipeline_tag": "text-generation"}

### 원문 설명
likes=260, downloads=21730, pipeline_tag=text-generation
...
### 핵심 포인트
- zai-org/GLM-5.2 모델의 높은 관심도(likes 1,792, downloads 27,413) 확인
- 약 753B 규모의 대규모 파라미터를 가진 text-generation 모델
- Hugging Face 내 모델 트렌딩 지표 반영

**태그**: GLM-5.2, text-generation, Large Language Model

**Metrics**: {"likes": 1792, "downloads": 27413, "num_parameters": 753329940480, "pipeline_tag": "text-generation"}

### 원문 설명
likes=1792, downloads=27413, pipeline_tag=text-generation
