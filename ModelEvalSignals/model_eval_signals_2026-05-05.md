# Model & Eval Signals (2026-05-05)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 함께 소프트웨어 엔지니어링 에이전트 역량 및 목표 지향적 개발 능력을 측정하기 위한 새로운 벤치마크들이 주목받고 있습니다.

### 오늘의 핵심 포인트
- Mistral-Medium-3.5-128B, Sulphur-2-base 등 다양한 규모와 목적(text-to-video 등)을 가진 모델들이 Hugging Face 트렌드 진입
- CodeClash, SWE-smith 등 단순 작업 수행을 넘어 목표 지향적 개발 및 에이전트 학습을 위한 새로운 평가 체계와 도구 등장
- METR를 통한 GPT-5, DeepSeek, Qwen 등 주요 모델들에 대한 심층 평가 리포트 지속 발표

**오늘의 태그**: LLM, Benchmark, AI Agents, Hugging Face, Software Engineering

## 1. [mistralai/Mistral-Medium-3.5-128B](https://huggingface.co/mistralai/Mistral-Medium-3.5-128B)
...
- 15,000건 이상의 높은 다운로드 수 기록

**태그**: Mistral-Medium-3.5-128B, Mistral-AI, LLM

**Metrics**: {"likes": 262, "downloads": 15024, "num_parameters": 127704211408, "pipeline_tag": ""}

### 원문 설명
likes=262, downloads=15024, pipeline_tag=
...
### 요약
Hugging Face에서 주목받고 있는 SulphurAI의 Sulphur-2-base 모델은 text-to-video 파이프라인을 지원하는 베이스 모델입니다. 약 8.95B 파라미터를 보유하고 있으며, 최근 높은 다운로드 수를 기록하며 트렌드에 진입했습니다.

### 핵심 포인트
- SulphurAI에서 공개한 Sulphur-2-base 모델
- text-to-video 태그를 가진 비디오 생성 관련 모델
- 약 8.95B 규모의 파라미터 수치

**태그**: Sulphur-2-base, text-to-video, SulphurAI

**Metrics**: {"likes": 205, "downloads": 37897, "num_parameters": 8953803264, "pipeline_tag": "text-to-video"}

### 원문 설명
likes=205, downloads=37897, pipeline_tag=text-to-video

---

## 3. [poolside/Laguna-XS.2](https://huggingface.co/poolside/Laguna-XS.2)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Hugging Face에서 높은 다운로드 수를 기록 중인 poolside/Laguna-XS.2 모델에 대한 동향입니다. 이 모델은 text-generation 파이프라인을 지원하는 약 33.4B 파라미터 규모의 모델입니다.

### 핵심 포인트
- poolside/Laguna-XS.2 모델의 높은 관심도(downloads 12,027회) 확인
- 약 33.4B 규모의 파라미터를 가진 text-generation 모델
- Hugging Face 내 모델 트렌딩 지표 반영

**태그**: poolside/Laguna-XS.2, text-generation, Hugging Face

**Metrics**: {"likes": 215, "downloads": 12027, "num_parameters": 33442617088, "pipeline_tag": "text-generation"}

### 원문 설명
likes=215, downloads=12027, pipeline_tag=text-generation
