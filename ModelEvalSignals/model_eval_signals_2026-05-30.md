# Model & Eval Signals (2026-05-30)

## 오늘의 요약
Hugging Face의 신규 모델 공개와 함께 소프트웨어 엔지니어링 및 코딩 역량 평가를 위한 새로운 벤치마크와 모델 성과가 주요 흐름을 형성하고 있습니다.

### 오늘의 핵심 포인트
- LiquidAI, ByteDance, NVIDIA 등 주요 기업의 특화 모델(Text-gen, Any-to-any, Image-text) 공개
- SWE-bench 팀의 목표 지향적 개발 역량 평가를 위한 CodeClash 및 에이전트 학습 환경 SWE-smith 출시
- LiveCodeBench를 통한 Gemini 및 O4-Mini 등 최신 모델의 코딩 성능 지표 확인

**오늘의 태그**: LLM, Benchmark, Software Engineering Agents, Multimodal, Coding Evaluation

## 1. [LiquidAI/LFM2.5-8B-A1B](https://huggingface.co/LiquidAI/LFM2.5-8B-A1B)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
LiquidAI에서 공개한 LFM2.5-8B-A1B 모델이 Hugging Face에서 주목받고 있습니다. 약 8.5B 파라미터를 가진 이 모델은 text-generation 태스크를 위한 모델입니다.

### 핵심 포인트
- LiquidAI에서 출시한 8.5B 규모의 text-generation 모델
- Hugging Face에서 8,854회의 다운로드를 기록하며 트렌딩 중
- 모델 파라미터 수는 약 8.47B 수준

**태그**: LiquidAI, LFM2.5-8B-A1B, text-generation

**Metrics**: {"likes": 194, "downloads": 8854, "num_parameters": 8467856832, "pipeline_tag": "text-generation"}

### 원문 설명
likes=194, downloads=8854, pipeline_tag=text-generation
...
- 2,738회의 다운로드와 972개의 좋아요를 기록하며 트렌딩 중

**태그**: ByteDance, Lance, any-to-any

**Metrics**: {"likes": 972, "downloads": 2738, "num_parameters": 0, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=972, downloads=2738, pipeline_tag=any-to-any
...
### 요약
NVIDIA에서 공개한 3B 규모의 image-text-to-text 모델인 nvidia/LocateAnything-3B가 Hugging Face에서 주목받고 있습니다. 약 3.8B 파라미터를 보유한 이 모델은 이미지와 텍스트를 결합하여 처리하는 태스크에 특화되어 있습니다.

### 핵심 포인트
- NVIDIA에서 출시한 3B 규모의 image-text-to-text 모델
- 약 3.8B의 파라미터 규모를 가진 멀티모달 모델
- Hugging Face에서 높은 다운로드 수(7,861회)와 좋아요(360개)를 기록하며 트렌딩 중

**태그**: NVIDIA, LocateAnything-3B, image-text-to-text

**Metrics**: {"likes": 360, "downloads": 7861, "num_parameters": 3830665968, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=360, downloads=7861, pipeline_tag=image-text-to-text
