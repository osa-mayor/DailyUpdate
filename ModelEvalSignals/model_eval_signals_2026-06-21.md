# Model & Eval Signals (2026-06-21)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 및 코딩 역량 중심의 벤치마크 업데이트가 주요 흐름을 형성했습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 VibeThinker-3B, GLM-5.2, Gemma-4 기반 GGUF 모델 등 다양한 규모와 목적의 텍스트 생성 모델들이 높은 관심을 받으며 트렌딩 중입니다.
- SWE-bench 팀에서 목표 지향적 개발 역량 평가를 위한 'CodeClash'를 공개하고, 에이전트 학습 환경인 'SWE-smith'를 출시하는 등 AI 에이전트 평가 및 학습 도구가 고도화되고 있습니다.
- LiveCodeBench를 통해 O4-Mini 및 Gemini 시리즈 모델들의 코딩 성능 스냅샷이 업데이트되며 모델 간 경쟁이 지속되고 있습니다.
...
### 요약
WeiboAI에서 공개한 VibeThinker-3B 모델이 Hugging Face에서 주목받고 있습니다. 약 3B 파라미터 규모의 text-generation 모델로, 높은 다운로드 수를 기록하며 트렌딩 모델로 부상했습니다.

### 핵심 포인트
- WeiboAI에서 개발한 3B 규모의 text-generation 모델
- 16,270회의 다운로드와 501개의 likes를 기록하며 높은 관심을 받음
- 약 3.08B의 파라미터를 보유한 경량 모델

**태그**: WeiboAI, VibeThinker-3B, text-generation

**Metrics**: {"likes": 501, "downloads": 16270, "num_parameters": 3085938688, "pipeline_tag": "text-generation"}

### 원문 설명
likes=501, downloads=16270, pipeline_tag=text-generation
...
### 핵심 포인트
- zai-org/GLM-5.2 모델의 Hugging Face 내 높은 관심도 확인
- 19,683회의 다운로드와 1,660개의 likes를 기록하며 트렌딩 중
- 약 753B 파라미터를 보유한 대규모 text-generation 모델

**태그**: GLM-5.2, text-generation, Hugging Face

**Metrics**: {"likes": 1660, "downloads": 19683, "num_parameters": 753329940480, "pipeline_tag": "text-generation"}

### 원문 설명
likes=1660, downloads=19683, pipeline_tag=text-generation
...
### 요약
yuxinlu1/gemma-4-12B-coder-fable5-composer2.5-v1-GGUF 모델이 Hugging Face에서 높은 관심을 받고 있습니다. 해당 모델은 text-generation 태그를 가진 12B 규모의 GGUF 양자화 모델입니다.

### 핵심 포인트
- 모델명: yuxinlu1/gemma-4-12B-coder-fable5-composer2.5-v1-GGUF
- 312,332회의 높은 downloads와 1,958개의 likes를 기록하며 트렌딩 중
- 약 11.9B 파라미터를 가진 text-generation용 모델

**태그**: Gemma-4, GGUF, Text-Generation

**Metrics**: {"likes": 1958, "downloads": 312332, "num_parameters": 11907350576, "pipeline_tag": "text-generation"}

### 원문 설명
likes=1958, downloads=312332, pipeline_tag=text-generation
