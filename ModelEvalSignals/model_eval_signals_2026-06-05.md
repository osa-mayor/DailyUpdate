# Model & Eval Signals (2026-06-05)

## 오늘의 요약
Google의 Gemma-4 시리즈와 LiquidAI의 신규 모델이 Hugging Face에서 주목받는 가운데, SWE-bench와 LiveCodeBench를 중심으로 소프트웨어 엔지니어링 및 코딩 역량 평가 벤치마크가 활발히 업데이트되었습니다.

### 오늘의 핵심 포인트
- Google의 gemma-4-12B 및 it 모델이 any-to-any 파이프라인을 특징으로 Hugging Face에서 높은 관심을 받음
- SWE-bench 팀에서 목표 지향적 개발 역량 측정을 위한 새로운 벤치마크 'CodeClash' 및 에이전트 학습 환경 'SWE-smith' 공개
- LiveCodeBench 스냅샷 결과, O4-Mini 및 Gemini-2.5 시리즈 모델들이 상위권 성적 기록

**오늘의 태그**: LLM, Software Engineering Agents, Benchmark Update, Coding Evaluation

## 1. [google/gemma-4-12B](https://huggingface.co/google/gemma-4-12B)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Google의 gemma-4-12B 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 약 12B 규모의 파라미터를 보유하고 있으며, any-to-any 파이프라인 태그를 특징으로 합니다.

### 핵심 포인트
- google/gemma-4-12B 모델의 Hugging Face 트렌딩 기록
- 약 11.96B 규모의 파라미터를 가진 모델
- any-to-any 파이프라인 태그를 통한 범용적 활용 가능성

**태그**: google, gemma-4-12B, any-to-any

**Metrics**: {"likes": 252, "downloads": 1978, "num_parameters": 11959730224, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=252, downloads=1978, pipeline_tag=any-to-any
...
### 요약
Google의 gemma-4-12B-it 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 약 12B 규모의 파라미터를 보유하고 있으며, any-to-any 파이프라인 태그가 특징입니다.

### 핵심 포인트
- google/gemma-4-12B-it 모델의 높은 관심도 반영 (downloads 14,866건)
- 약 11.96B 규모의 파라미터를 가진 모델
- any-to-any 파이프라인 태그를 통한 범용적 활용 가능성

**태그**: google, gemma-4-12B-it, LLM

**Metrics**: {"likes": 377, "downloads": 14866, "num_parameters": 11959730224, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=377, downloads=14866, pipeline_tag=any-to-any
...
### 핵심 포인트
- LiquidAI의 LFM2.5-8B-A1B 모델이 Hugging Face에서 높은 관심을 받고 있음
- 72,114회의 다운로드와 505개의 likes를 기록함
- 약 8.47B 파라미터를 보유한 text-generation 모델임

**태그**: LiquidAI, LFM2.5-8B-A1B, text-generation

**Metrics**: {"likes": 505, "downloads": 72114, "num_parameters": 8467856832, "pipeline_tag": "text-generation"}

### 원문 설명
likes=505, downloads=72114, pipeline_tag=text-generation
