# Model & Eval Signals (2026-05-13)

## 오늘의 요약
Hugging Face를 중심으로 한 신규 멀티모달 및 고성능 모델들의 등장과 함께, 소프트웨어 엔지니어링 에이전트의 역량을 측정하고 학습시키기 위한 새로운 벤치마크 및 도구들이 주목받고 있습니다.

### 오늘의 핵심 포인트
- MiniCPM-V-4.6, ZAYA1-8B, gemma-4-31B 등 다양한 목적의 신규 모델들이 Hugging Face에서 높은 관심을 받음
- CodeClash, SWE-smith 등 소프트웨어 엔지니어링 에이전트의 목표 지향적 역량과 학습을 위한 새로운 평가 체계 등장
- METR를 통한 GPT-5, DeepSeek, Qwen 등 주요 모델들에 대한 심도 있는 평가 리포트 지속 발표
...
### 요약
OpenBMB에서 공개한 MiniCPM-V-4.6 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 멀티모달 모델입니다.

### 핵심 포인트
- OpenBMB의 MiniCPM-V-4.6 모델 출시
- image-text-to-text 태스크를 지원하는 파이프라인
- 약 1.3B 규모의 파라미터를 보유한 모델

**태그**: MiniCPM-V-4.6, OpenBMB, image-text-to-text

**Metrics**: {"likes": 387, "downloads": 0, "num_parameters": 1300428016, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=387, downloads=0, pipeline_tag=image-text-to-text
...
### 요약
Zyphra에서 공개한 ZAYA1-8B 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 약 8.8B 파라미터를 보유한 이 모델은 최근 사용자들 사이에서 활발하게 활용되고 있습니다.

### 핵심 포인트
- Zyphra의 ZAYA1-8B 모델이 높은 다운로드 수(66,119회)를 기록하며 트렌딩 중입니다.
- 모델의 파라미터 규모는 약 8.8B입니다.
- Hugging Face 내에서 448개의 likes를 확보하며 사용자 관심을 끌고 있습니다.

**태그**: Zyphra, ZAYA1-8B, Model Trending

**Metrics**: {"likes": 448, "downloads": 66119, "num_parameters": 8840488784, "pipeline_tag": ""}

### 원문 설명
likes=448, downloads=66119, pipeline_tag=

---

## 3. [google/gemma-4-31B-it-assistant](https://huggingface.co/google/gemma-4-31B-it-assistant)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Google의 gemma-4-31B-it-assistant 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 any-to-any 파이프라인 태그를 보유하고 있습니다.

### 핵심 포인트
- google/gemma-4-31B-it-assistant 모델의 높은 관심도 반영
- 66,561회의 다운로드와 215개의 likes 기록
- any-to-any 파이프라인 태그를 통한 범용적 활용 가능성

**태그**: google, gemma-4-31B-it-assistant, any-to-any

**Metrics**: {"likes": 215, "downloads": 66561, "num_parameters": 469518596, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=215, downloads=66561, pipeline_tag=any-to-any
