# Model & Eval Signals (2026-06-12)

## 오늘의 요약
Hugging Face의 신규 모델 공개와 함께 소프트웨어 엔지니어링 에이전트 역량 및 코딩 성능을 측정하는 새로운 벤치마크/결과들이 주목받고 있습니다.

### 오늘의 핵심 포인트
- CohereLabs, BosonAI, Google 등 주요 기업의 특화 모델(Code, TTS, Multimodal) 공개
- 단순 작업 수행을 넘어 목표 지향적 개발 역량을 평가하는 CodeClash 등 새로운 벤치마크 등장
- mini-SWE-agent 및 Gemini 시리즈 등 코딩 및 에이전트 성능을 보여주는 벤치마크 결과 업데이트

**오늘의 태그**: LLM, Software Engineering Agents, Coding Benchmark, Hugging Face, AI Evaluation
...
### 핵심 포인트
- CohereLabs에서 출시한 North-Mini-Code-1.0 모델
- 약 30.5B(30,484,303,872) 규모의 파라미터를 보유
- text-generation 파이프라인을 지원하며 높은 다운로드 수를 기록 중

**태그**: CohereLabs, North-Mini-Code-1.0, text-generation

**Metrics**: {"likes": 302, "downloads": 1859, "num_parameters": 30484303872, "pipeline_tag": "text-generation"}

### 원문 설명
likes=302, downloads=1859, pipeline_tag=text-generation

---

## 2. [bosonai/higgs-audio-v3-tts-4b](https://huggingface.co/bosonai/higgs-audio-v3-tts-4b)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending
...
- 높은 다운로드 수(19,948회)를 기록하며 사용자 관심을 확인

**태그**: bosonai, text-to-speech, TTS

**Metrics**: {"likes": 349, "downloads": 19948, "num_parameters": 4654850537, "pipeline_tag": "text-to-speech"}

### 원문 설명
likes=349, downloads=19948, pipeline_tag=text-to-speech
...
### 요약
Google에서 공개한 diffusiongemma-26B-A4B-it 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 26B 규모의 모델입니다.

### 핵심 포인트
- Google의 diffusiongemma-26B-A4B-it 모델 공개
- image-text-to-text 태그를 가진 멀티모달 성격의 모델
- 약 25.8B 규모의 파라미터를 보유

**태그**: Google, diffusiongemma, image-text-to-text

**Metrics**: {"likes": 453, "downloads": 0, "num_parameters": 25823778864, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=453, downloads=0, pipeline_tag=image-text-to-text
