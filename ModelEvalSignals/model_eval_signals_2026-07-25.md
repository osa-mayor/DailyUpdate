# Model & Eval Signals (2026-07-25)

## 오늘의 요약
Hugging Face의 신규 모델 공개와 더불어, 소프트웨어 엔지니어링 에이전트 역량 측정 및 학습을 위한 새로운 벤치마크와 도구들이 등장하며 에이전트 중심의 평가 체계가 강화되고 있습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 Upstage의 Solar-Open2-250B 등 다양한 규모의 텍스트 생성 모델들이 주목받음
- 목표 지향적 개발자 역량 측정을 위한 CodeClash 및 에이전트 학습 도구 SWE-smith 등 새로운 평가/학습 프레임워크 등장
- LiveCodeBench를 통해 O4-Mini 및 Gemini 시리즈 등 최신 모델들의 코딩 성능 지표 업데이트
...
### 핵심 포인트
- poolside/Laguna-S-2.1 모델의 높은 사용자 관심도 반영 (downloads 28,992회)
- 약 117.5B 파라미터를 보유한 대규모 모델
- text-generation 태그를 사용하는 텍스트 생성 특화 모델

**태그**: poolside/Laguna-S-2.1, text-generation, LLM

**Metrics**: {"likes": 586, "downloads": 28992, "num_parameters": 117561977600, "pipeline_tag": "text-generation"}

### 원문 설명
likes=586, downloads=28992, pipeline_tag=text-generation
...
### 핵심 포인트
- Nanbeige/Nanbeige4.2-3B 모델의 높은 관심도 확인
- 8,169회의 downloads와 358회의 likes 기록
- 4.17B 규모의 파라미터를 보유한 text-generation 모델

**태그**: Nanbeige, text-generation, LLM

**Metrics**: {"likes": 358, "downloads": 8169, "num_parameters": 4169800704, "pipeline_tag": "text-generation"}

### 원문 설명
likes=358, downloads=8169, pipeline_tag=text-generation
...
- likes 534회, downloads 1106회를 기록하며 높은 관심을 받음

**태그**: Upstage, Solar-Open2-250B, text-generation

**Metrics**: {"likes": 534, "downloads": 1106, "num_parameters": 250287810304, "pipeline_tag": "text-generation"}

### 원문 설명
likes=534, downloads=1106, pipeline_tag=text-generation
