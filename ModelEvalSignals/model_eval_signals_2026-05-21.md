# Model & Eval Signals (2026-05-21)

## 오늘의 요약
ByteDance와 unsloth의 새로운 모델 공개로 인한 멀티모달 및 효율적 모델 트렌드와 함께, SWE-bench 및 LiveCodeBench를 통한 에이전트 및 코딩 역량 평가 벤치마크의 진화가 두드러진 하루였습니다.

### 오늘의 핵심 포인트
- ByteDance의 any-to-any 모델 'Lance' 및 unsloth의 Qwen3.6 기반 GGUF 모델 등 고성능/고효율 모델 트렌드 지속
- 단순 작업 수행을 넘어 목표 지향적 개발 역량을 측정하는 'CodeClash' 등 소프트웨어 엔지니어링 에이전트 평가 체계의 고도화
- LiveCodeBench를 통해 확인된 Gemini 및 O4-Mini 등 최신 모델들의 코딩 성능 경쟁

**오늘의 태그**: LLM, Multi-modal, Software Engineering Agents, Coding Benchmark, Model Trending

## 1. [bytedance-research/Lance](https://huggingface.co/bytedance-research/Lance)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
ByteDance Research에서 공개한 any-to-any 모델인 Lance가 Hugging Face에서 주목받고 있습니다. 해당 모델은 다양한 입력과 출력을 지원하는 pipeline_tag를 특징으로 합니다.

### 핵심 포인트
- ByteDance Research에서 개발한 Lance 모델 공개
- any-to-any 파이프라인 태그를 지원하는 모델
- Hugging Face 내 likes 446회, downloads 438회를 기록하며 트렌딩 중

**태그**: ByteDance, Lance, any-to-any

**Metrics**: {"likes": 446, "downloads": 438, "num_parameters": 0, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=446, downloads=438, pipeline_tag=any-to-any
...
### 핵심 포인트
- unsloth/Qwen3.6-27B-MTP-GGUF 모델의 높은 다운로드 수(411,598회) 기록
- 27B 규모의 파라미터를 가진 모델
- image-text-to-text 파이프라인 태그를 보유한 멀티모달 성격의 모델

**태그**: unsloth, Qwen3.6, GGUF

**Metrics**: {"likes": 351, "downloads": 411598, "num_parameters": 27320697856, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=351, downloads=411598, pipeline_tag=image-text-to-text
...
### 요약
unsloth에서 공개한 Qwen3.6-35B-A3B-MTP-GGUF 모델이 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 GGUF 포맷 모델입니다.

### 핵심 포인트
- unsloth에서 배포한 Qwen3.6-35B-A3B-MTP-GGUF 모델
- 363,131회의 높은 다운로드 수 기록
- image-text-to-text 파이프라인을 지원하는 35B 규모의 모델

**태그**: unsloth, Qwen3.6, GGUF

**Metrics**: {"likes": 289, "downloads": 363131, "num_parameters": 35505251456, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=289, downloads=363131, pipeline_tag=image-text-to-text
