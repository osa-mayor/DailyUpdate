# Model & Eval Signals (2026-06-13)

## 오늘의 요약
멀티모달 모델(Image-to-Text)의 활발한 공개와 더불어, 소프트웨어 엔지니어링 에이전트의 역량을 측정하기 위한 새로운 벤치마크 및 학습 환경이 등장하며 에이전트 중심의 평가 체계가 고도화되고 있습니다.

### 오늘의 핵심 포인트
- Unsloth, NVIDIA, Moonshot AI 등에서 이미지-텍스트 통합 파이프라인을 지원하는 멀티모달 모델들이 높은 관심을 받으며 공개됨
- SWE-bench 팀의 CodeClash 출시 및 mini-SWE-agent의 성과를 통해 목표 지향적 개발 에이전트 평가 및 구현 기술이 주목받음
- LiveCodeBench를 통해 O4-Mini 및 Gemini 시리즈 등 최신 모델들의 코딩 성능 스냅샷이 업데이트됨

**오늘의 태그**: Multimodal, AI Agents, Software Engineering, LLM Benchmarks
...
### 핵심 포인트
- unsloth/diffusiongemma-26B-A4B-it-GGUF 모델의 높은 관심도(downloads 17,666회)
- image-text-to-text 작업을 위한 파이프라인 지원
- 약 25.25B 파라미터를 보유한 모델 규모

**태그**: unsloth, diffusiongemma, GGUF

**Metrics**: {"likes": 208, "downloads": 17666, "num_parameters": 25250987068, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=208, downloads=17666, pipeline_tag=image-text-to-text
...
### 요약
NVIDIA에서 공개한 3B 규모의 image-text-to-text 모델인 nvidia/LocateAnything-3B가 Hugging Face에서 주목받고 있습니다. 약 15만 건에 달하는 높은 다운로드 수를 기록하며 기술적 관심을 모으고 있습니다.

### 핵심 포인트
- NVIDIA에서 출시한 3B 파라미터 규모의 image-text-to-text 모델입니다.
- 149,206회의 다운로드와 1,916개의 likes를 기록하며 높은 트렌드 지표를 보이고 있습니다.
- 이미지와 텍스트를 결합하여 처리하는 멀티모달 파이프라인을 지원합니다.

**태그**: NVIDIA, LocateAnything-3B, image-text-to-text

**Metrics**: {"likes": 1916, "downloads": 149206, "num_parameters": 3830665968, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1916, downloads=149206, pipeline_tag=image-text-to-text
...
### 요약
Moonshot AI에서 공개한 Kimi-K2.7-Code 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 대규모 모델입니다.

### 핵심 포인트
- Moonshot AI의 Kimi-K2.7-Code 모델이 Hugging Face 트렌딩에 진입했습니다.
- 모델의 pipeline_tag는 image-text-to-text로 분류됩니다.
- 약 1.06T(1,058,589,420,528) 규모의 파라미터를 보유하고 있습니다.

**태그**: MoonshotAI, Kimi-K2.7-Code, Multimodal

**Metrics**: {"likes": 302, "downloads": 0, "num_parameters": 1058589420528, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=302, downloads=0, pipeline_tag=image-text-to-text
