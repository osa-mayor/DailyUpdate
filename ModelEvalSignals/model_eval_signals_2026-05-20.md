# Model & Eval Signals (2026-05-20)

## 오늘의 요약
Hugging Face의 신규 멀티모달 및 음성 합성 모델들이 트렌딩되는 가운데, SWE-bench와 LiveCodeBench를 중심으로 소프트웨어 엔지니어링 에이전트 및 코딩 능력 평가에 대한 기술적 진보가 두드러졌습니다.

### 오늘의 핵심 포인트
- ByteDance의 Lance, openbmb의 MiniCPM-V 등 멀티모달 및 any-to-any 모델의 높은 관심
- 목표 지향적 개발 역량 측정을 위한 CodeClash 및 에이전트 학습 환경 SWE-smith 등 소프트웨어 엔지니어링 평가 도구의 진화
- LiveCodeBench를 통한 Gemini 및 O4-Mini 등 최신 모델들의 코딩 성능 벤치마크 경쟁

**오늘의 태그**: Multimodal, Software Engineering Agents, Coding Benchmark, LLM Evaluation

## 1. [bytedance-research/Lance](https://huggingface.co/bytedance-research/Lance)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
ByteDance Research에서 공개한 any-to-any 모델인 Lance가 Hugging Face에서 주목받고 있습니다. 현재 높은 좋아요 수와 다운로드 수를 기록하며 트렌딩 모델로 부상했습니다.

### 핵심 포인트
- ByteDance Research에서 개발한 Lance 모델이 공개되었습니다.
- 모델의 pipeline_tag는 any-to-any로 분류됩니다.
- 현재 Hugging Face에서 likes 286회, downloads 171회를 기록하며 트렌딩 중입니다.

**태그**: ByteDance, Lance, any-to-any

**Metrics**: {"likes": 286, "downloads": 171, "num_parameters": 0, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=286, downloads=171, pipeline_tag=any-to-any
...
### 요약
openbmb에서 공개한 MiniCPM-V-4.6 모델이 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 멀티모달 모델입니다.

### 핵심 포인트
- openbmb/MiniCPM-V-4.6 모델의 높은 관심도(likes 800, downloads 144,826) 확인
- image-text-to-text 작업을 수행하는 멀티모달 파이프라인 제공
- 약 1.3B(1,300,428,016) 규모의 파라미터를 가진 모델

**태그**: MiniCPM-V-4.6, openbmb, image-text-to-text

**Metrics**: {"likes": 800, "downloads": 144826, "num_parameters": 1300428016, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=800, downloads=144826, pipeline_tag=image-text-to-text
...
### 요약
Supertone에서 공개한 text-to-speech 모델인 supertonic-3가 Hugging Face에서 주목받고 있습니다. 높은 다운로드 수를 기록하며 음성 합성 분야에서 사용자들의 관심을 끌고 있습니다.

### 핵심 포인트
- Supertone의 text-to-speech 모델인 supertonic-3가 공개됨
- 28,681회의 높은 downloads를 기록하며 트렌딩 중
- 460개의 likes를 확보하며 사용자 피드백을 얻고 있음

**태그**: Supertone, supertonic-3, text-to-speech

**Metrics**: {"likes": 460, "downloads": 28681, "num_parameters": 0, "pipeline_tag": "text-to-speech"}

### 원문 설명
likes=460, downloads=28681, pipeline_tag=text-to-speech
