# Model & Eval Signals (2026-05-19)

## 오늘의 요약
Hugging Face를 중심으로 한 신규 모델(MiniCPM-V, Supertonic-3 등)의 등장과 함께, 소프트웨어 엔지니어링 에이전트 역량 및 목표 지향적 개발 능력을 측정하기 위한 새로운 벤치마크(CodeClash, SWE-smith)가 주목받고 있습니다.

### 오늘의 핵심 포인트
- OpenBMB, Supertone 등 다양한 도메인(VLM, TTS)의 신규 모델이 Hugging Face 트렌드 주도
- 단순 작업 수행을 넘어 목표 지향적 개발 역량을 평가하는 CodeClash 및 에이전트 학습 환경인 SWE-smith 등 새로운 벤치마크 등장
- METR를 통한 GPT-5, DeepSeek, Qwen 등 주요 모델에 대한 지속적인 평가 리포트 발행
...
### 요약
OpenBMB에서 공개한 MiniCPM-V-4.6 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 약 1.3B 파라미터를 가진 image-text-to-text 태스크용 모델입니다.

### 핵심 포인트
- OpenBMB의 MiniCPM-V-4.6 모델이 높은 다운로드 수를 기록하며 트렌딩 중입니다.
- 약 1.3B 규모의 파라미터를 보유한 경량급 모델입니다.
- 이미지와 텍스트를 동시에 처리하는 image-text-to-text 파이프라인을 지원합니다.

**태그**: MiniCPM-V-4.6, OpenBMB, image-text-to-text

**Metrics**: {"likes": 766, "downloads": 80586, "num_parameters": 1300428016, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=766, downloads=80586, pipeline_tag=image-text-to-text

---

## 2. [Supertone/supertonic-3](https://huggingface.co/Supertone/supertonic-3)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Supertone에서 공개한 text-to-speech 모델인 supertonic-3가 Hugging Face에서 주목받고 있습니다. 해당 모델은 높은 다운로드 수를 기록하며 TTS 분야에서 트렌드를 형성하고 있습니다.

### 핵심 포인트
- Supertone의 text-to-speech 모델인 supertonic-3가 공개됨
- 24,031회의 높은 다운로드 수를 기록하며 사용자 관심을 모음
- 413개의 likes를 기록하며 모델 성능 및 활용도에 대한 긍정적 신호를 보임

**태그**: Supertone, supertonic-3, text-to-speech

**Metrics**: {"likes": 413, "downloads": 24031, "num_parameters": 0, "pipeline_tag": "text-to-speech"}

### 원문 설명
likes=413, downloads=24031, pipeline_tag=text-to-speech
...
### 요약
unsloth/Qwen3.6-27B-MTP-GGUF 모델이 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 27B 규모의 모델입니다.

### 핵심 포인트
- 268,305회의 높은 downloads를 기록하며 트렌딩 중
- image-text-to-text 파이프라인 태그를 가진 멀티모달 모델
- 약 27.3B 규모의 파라미터를 보유한 GGUF 포맷 모델

**태그**: unsloth, Qwen3.6-27B-MTP-GGUF, image-text-to-text

**Metrics**: {"likes": 279, "downloads": 268305, "num_parameters": 27320697856, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=279, downloads=268305, pipeline_tag=image-text-to-text
