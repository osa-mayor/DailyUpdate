# Model & Eval Signals (2026-07-22)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 에이전트 및 코딩 성능을 측정하는 벤치마크 업데이트가 주요 흐름을 형성했습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 OCR 및 Image-to-Text 파이프라인을 지원하는 신규 모델들이 높은 관심을 받으며 트렌드를 주도함
...
**오늘의 태그**: HuggingFace, SWE-bench, LiveCodeBench, AI_Agents, OCR

## 1. [conradlocke/krea2-identity-edit](https://huggingface.co/conradlocke/krea2-identity-edit)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
conradlocke/krea2-identity-edit 모델이 Hugging Face에서 주목받고 있습니다. 해당 모델은 471개의 likes를 기록하며 사용자들의 관심을 끌고 있습니다.

### 핵심 포인트
- conradlocke/krea2-identity-edit 모델의 Hugging Face 트렌드 발생
- 471개의 likes를 기록하며 사용자 관심을 유도
- 현재 downloads는 0으로 기록됨

**태그**: conradlocke/krea2-identity-edit, Model Trending, Hugging Face

**Metrics**: {"likes": 471, "downloads": 0, "num_parameters": 0, "pipeline_tag": ""}

### 원문 설명
likes=471, downloads=0, pipeline_tag=
...
- 220만 회 이상의 다운로드와 2,500개 이상의 likes 기록
- image-text-to-text 파이프라인을 활용한 OCR 작업 수행

**태그**: Baidu, Unlimited-OCR, OCR

**Metrics**: {"likes": 2586, "downloads": 2237351, "num_parameters": 3336106240, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=2586, downloads=2237351, pipeline_tag=image-text-to-text
...
### 요약
Hugging Face에서 높은 관심을 받고 있는 thinkingmachines/Inkling 모델에 대한 동향입니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 대규모 모델입니다.

### 핵심 포인트
- thinkingmachines/Inkling 모델이 16,441회의 downloads와 1,341회의 likes를 기록하며 주목받고 있습니다.
- 해당 모델의 pipeline_tag는 image-text-to-text로 분류됩니다.
- 모델의 파라미터 규모는 약 952B 수준입니다.

**태그**: thinkingmachines/Inkling, image-text-to-text, Hugging Face

**Metrics**: {"likes": 1341, "downloads": 16441, "num_parameters": 952377623626, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1341, downloads=16441, pipeline_tag=image-text-to-text
