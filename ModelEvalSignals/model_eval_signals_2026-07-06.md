# Model & Eval Signals (2026-07-06)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 및 코딩 역량 중심의 벤치마크 업데이트가 주요 흐름을 형성했습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 정형 데이터 분류(google/tabfm) 및 대규모 언어 모델(DeepSeek, InternScience)의 신규 모델 공개 및 트렌드 진입
- SWE-bench의 새로운 평가 방식(CodeClash) 및 에이전트 학습 환경(SWE-smith) 등 소프트웨어 엔지니어링 에이전트 역량 강화 도구 등장
- LiveCodeBench를 통한 Gemini 및 O4-Mini 등 주요 모델의 코딩 성능 스냅샷 업데이트

**오늘의 태그**: LLM, Software Engineering Agents, Benchmark, Hugging Face, Coding Capability

## 1. [google/tabfm-1.0.0-pytorch](https://huggingface.co/google/tabfm-1.0.0-pytorch)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Google에서 공개한 tabular-classification 태스크용 모델인 google/tabfm-1.0.0-pytorch가 Hugging Face에서 주목받고 있습니다. 해당 모델은 정형 데이터 분류를 목적으로 하며, 최근 다운로드 수가 증가하며 트렌드에 진입했습니다.

### 핵심 포인트
- google/tabfm-1.0.0-pytorch 모델의 Hugging Face 트렌드 발생
- pipeline_tag는 tabular-classification으로 정형 데이터 분류 작업에 특화됨
- 2,670회의 다운로드와 219개의 likes를 기록 중

**태그**: google, tabular-classification, pytorch

**Metrics**: {"likes": 219, "downloads": 2670, "num_parameters": 0, "pipeline_tag": "tabular-classification"}

### 원문 설명
likes=219, downloads=2670, pipeline_tag=tabular-classification
...
### 요약
DeepSeek-AI에서 공개한 DeepSeek-V4-Pro-DSpark 모델이 Hugging Face에서 주목받고 있습니다. 해당 모델은 text-generation 태그로 분류된 대규모 언어 모델입니다.

### 핵심 포인트
- DeepSeek-AI의 text-generation 모델인 DeepSeek-V4-Pro-DSpark가 트렌딩 중입니다.
- 모델의 다운로드 수가 12,580회를 기록하며 높은 관심을 받고 있습니다.
- 약 889B(889,484,881,098) 규모의 파라미터를 가진 모델입니다.

**태그**: DeepSeek-V4-Pro-DSpark, text-generation, Hugging Face

**Metrics**: {"likes": 382, "downloads": 12580, "num_parameters": 889484881098, "pipeline_tag": "text-generation"}

### 원문 설명
likes=382, downloads=12580, pipeline_tag=text-generation
...
### 요약
InternScience에서 공개한 Agents-A1 모델이 Hugging Face에서 주목받고 있습니다. 해당 모델은 text-generation 파이프라인을 지원하는 약 35B 규모의 모델입니다.

### 핵심 포인트
- InternScience의 Agents-A1 모델이 Hugging Face에서 트렌딩 중입니다.
- 모델의 파라미터 규모는 약 35.1B입니다.
- text-generation 태그를 가진 모델로, 7,010회의 다운로드를 기록했습니다.

**태그**: Agents-A1, text-generation, HuggingFace

**Metrics**: {"likes": 279, "downloads": 7010, "num_parameters": 35107181936, "pipeline_tag": "text-generation"}

### 원문 설명
likes=279, downloads=7010, pipeline_tag=text-generation
