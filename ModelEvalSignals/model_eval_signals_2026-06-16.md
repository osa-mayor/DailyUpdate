# Model & Eval Signals (2026-06-16)

## 오늘의 요약
Hugging Face를 통한 신규 멀티모달 및 코드 특화 모델의 활발한 공개와 더불어, 소프트웨어 엔지니어링 에이전트의 역량을 측정하기 위한 새로운 벤치마크 및 평가 방법론이 주목받고 있습니다.

### 오늘의 핵심 포인트
- Moonshot AI, MiniMaxAI 등 신규 모델 공개로 멀티모달 및 코드 생성 기능 강화
- CodeClash, SWE-smith 등 목표 지향적 개발 역량 및 에이전트 학습을 위한 새로운 평가 도구 등장
- LiveCodeBench를 통한 최신 모델(O4-Mini, Gemini 시리즈)의 코딩 성능 스냅샷 업데이트

**오늘의 태그**: LLM, Multimodal, Code Generation, AI Agents, Benchmark

## 1. [moonshotai/Kimi-K2.7-Code](https://huggingface.co/moonshotai/Kimi-K2.7-Code)
**Source**: HF Model Trending | **Signal Type**: model_trending | **Category**: Model Trending

### 요약
Moonshot AI에서 공개한 Kimi-K2.7-Code 모델이 Hugging Face에서 높은 다운로드 수를 기록하며 주목받고 있습니다. 이 모델은 image-text-to-text 파이프라인을 지원하는 것이 특징입니다.

### 핵심 포인트
- moonshotai/Kimi-K2.7-Code 모델의 Hugging Face 트렌딩 기록
- 56,750회의 높은 다운로드 수와 724개의 likes 기록
- image-text-to-text 파이프라인을 지원하는 멀티모달 성격의 모델

**태그**: moonshotai, Kimi-K2.7-Code, image-text-to-text

**Metrics**: {"likes": 724, "downloads": 56750, "num_parameters": 1058589420528, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=724, downloads=56750, pipeline_tag=image-text-to-text
...
### 핵심 포인트
- MiniMax-M3 모델의 Hugging Face 내 높은 관심도(likes 787, downloads 14,312) 확인
- image-text-to-text 파이프라인을 지원하는 멀티모달 모델
- 약 427B 규모의 파라미터를 보유한 대형 모델

**태그**: MiniMax-M3, Multimodal, Hugging Face

**Metrics**: {"likes": 787, "downloads": 14312, "num_parameters": 427040140160, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=787, downloads=14312, pipeline_tag=image-text-to-text
...
- 11,145회의 downloads와 385회의 likes를 기록하며 트렌딩 중

**태그**: CohereLabs, North-Mini-Code-1.0, text-generation

**Metrics**: {"likes": 385, "downloads": 11145, "num_parameters": 30484303872, "pipeline_tag": "text-generation"}

### 원문 설명
likes=385, downloads=11145, pipeline_tag=text-generation
