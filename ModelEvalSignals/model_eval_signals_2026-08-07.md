# Model & Eval Signals (2026-08-07)

## 오늘의 요약
Hugging Face의 신규 모델 배포와 소프트웨어 엔지니어링 에이전트 역량 측정을 위한 새로운 벤치마크 및 성과가 주목받고 있습니다.

### 오늘의 핵심 포인트
- LiquidAI 및 MiniMaxAI의 신규 모델(LFM2.5, MiniMax-H3)이 텍스트 생성 및 비디오 생성 분야에서 높은 관심을 받으며 배포됨
- SWE-bench 팀이 목표 지향적 개발자 역량 측정을 위한 'CodeClash'를 도입하고, mini-SWE-agent의 성과 및 SWE-smith 출시 등 에이전트 학습 환경이 확장됨
...
### 핵심 포인트
- LiquidAI에서 공개한 2.6B 규모의 text-generation 모델
- 73,573회의 높은 다운로드 수를 기록하며 트렌드 형성
- 약 2.7B(2,697,198,592) 파라미터를 보유한 모델

**태그**: LiquidAI, LFM2.5-2.6B, text-generation

**Metrics**: {"likes": 323, "downloads": 73573, "num_parameters": 2697198592, "pipeline_tag": "text-generation"}

### 원문 설명
likes=323, downloads=73573, pipeline_tag=text-generation
...
### 핵심 포인트
- larryvrh/MiniMax-H3-Turbo-Lora 모델의 likes가 263을 기록하며 트렌딩되었습니다.
- 해당 모델의 pipeline_tag는 text-to-video입니다.
- 현재 downloads는 0으로 기록되어 있습니다.

**태그**: MiniMax-H3-Turbo-Lora, text-to-video, LoRA

**Metrics**: {"likes": 263, "downloads": 0, "num_parameters": 0, "pipeline_tag": "text-to-video"}

### 원문 설명
likes=263, downloads=0, pipeline_tag=text-to-video
...
### 핵심 포인트
- MiniMax-H3 모델은 12,102회의 다운로드를 기록하며 트렌드에 진입했습니다.
- 해당 모델의 주요 기능은 image-text-to-video 파이프라인입니다.
- 모델의 파라미터 규모는 약 33.1B입니다.

**태그**: MiniMax-H3, image-text-to-video, MiniMaxAI

**Metrics**: {"likes": 2708, "downloads": 12102, "num_parameters": 33122992896, "pipeline_tag": "image-text-to-video"}

### 원문 설명
likes=2708, downloads=12102, pipeline_tag=image-text-to-video
