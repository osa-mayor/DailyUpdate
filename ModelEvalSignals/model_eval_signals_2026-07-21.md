# Model & Eval Signals (2026-07-21)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 에이전트(SWE-bench) 및 코딩 성능(LiveCodeBench) 중심의 벤치마크 업데이트가 주를 이루었습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 이미지 편집, 멀티모달, GGUF 포맷 등 다양한 목적의 모델들이 주목받고 있습니다.
- SWE-bench 관련하여 목표 지향적 개발자 평가를 위한 CodeClash 출시 및 에이전트 학습 도구인 SWE-smith가 공개되었습니다.
- LiveCodeBench에서 O4-Mini 및 Gemini 시리즈 모델들이 상위권 성적을 기록하며 코딩 능력을 입증했습니다.
...
### 요약
conradlocke/krea2-identity-edit 모델이 Hugging Face에서 주목받고 있습니다. 해당 모델은 453개의 likes를 기록하며 사용자들의 관심을 끌고 있습니다.

### 핵심 포인트
- conradlocke/krea2-identity-edit 모델의 Hugging Face 트렌드 발생
- 453개의 likes를 기록하며 사용자 관심을 확보
- 현재 downloads는 0으로 기록됨

**태그**: conradlocke/krea2-identity-edit, Model Trending, Hugging Face

**Metrics**: {"likes": 453, "downloads": 0, "num_parameters": 0, "pipeline_tag": ""}

### 원문 설명
likes=453, downloads=0, pipeline_tag=
...
### 핵심 포인트
- thinkingmachines/Inkling 모델이 높은 likes와 downloads를 기록하며 주목받고 있습니다.
- 해당 모델의 pipeline_tag는 image-text-to-text로 분류됩니다.
- 모델의 규모는 약 952B 파라미터 수준의 대형 모델입니다.

**태그**: thinkingmachines/Inkling, image-text-to-text, Hugging Face

**Metrics**: {"likes": 1238, "downloads": 13462, "num_parameters": 952377623626, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=1238, downloads=13462, pipeline_tag=image-text-to-text
...
### 요약
prism-ml/Ternary-Bonsai-27B-gguf 모델이 높은 다운로드 수를 기록하며 주목받고 있습니다. 텍스트 생성 작업을 위한 GGUF 포맷 모델로, 약 3.6B 파라미터를 보유하고 있습니다.

### 핵심 포인트
- prism-ml/Ternary-Bonsai-27B-gguf 모델의 높은 다운로드 수(338,945회) 기록
- text-generation 파이프라인을 지원하는 GGUF 포맷 모델
- 약 3.6B 규모의 파라미터를 가진 모델

**태그**: Ternary-Bonsai-27B-gguf, text-generation, GGUF

**Metrics**: {"likes": 842, "downloads": 338945, "num_parameters": 3645856513, "pipeline_tag": "text-generation"}

### 원문 설명
likes=842, downloads=338945, pipeline_tag=text-generation
