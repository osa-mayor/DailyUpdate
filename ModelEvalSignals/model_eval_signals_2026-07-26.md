# Model & Eval Signals (2026-07-26)

## 오늘의 요약
Hugging Face의 신규 모델 트렌드와 소프트웨어 엔지니어링 에이전트 역량 강화를 위한 새로운 벤치마크 및 도구들이 주목받고 있습니다.

### 오늘의 핵심 포인트
- Hugging Face에서 Nanbeige, poolside, Upstage의 Solar 모델 등 다양한 규모의 텍스트 생성 모델들이 높은 다운로드 수를 기록하며 트렌드에 진입함
- SWE-bench 팀이 목표 지향적 개발자 역량 측정을 위한 CodeClash를 도입하고, mini-SWE-agent 및 SWE-smith 등 에이전트 관련 성과와 도구가 발표됨
...
### 요약
Nanbeige/Nanbeige4.2-3B 모델이 Hugging Face에서 텍스트 생성 태그와 함께 주목받고 있습니다. 약 4.17B 파라미터를 가진 이 모델은 최근 높은 다운로드 수를 기록하며 트렌드에 진입했습니다.

### 핵심 포인트
- Nanbeige/Nanbeige4.2-3B 모델의 Hugging Face 트렌드 진입
- 약 4.17B 규모의 파라미터를 보유한 text-generation 모델
- 11,573회의 다운로드와 395개의 likes를 기록하며 높은 관심을 받음

**태그**: Nanbeige, text-generation, LLM

**Metrics**: {"likes": 395, "downloads": 11573, "num_parameters": 4169800704, "pipeline_tag": "text-generation"}

### 원문 설명
likes=395, downloads=11573, pipeline_tag=text-generation
...
### 핵심 포인트
- poolside/Laguna-S-2.1 모델의 높은 사용자 관심도 반영 (downloads 45,260회)
- 약 117.5B 파라미터를 보유한 대규모 모델
- text-generation 태스크를 위한 모델로 분류됨

**태그**: poolside/Laguna-S-2.1, text-generation, LLM

**Metrics**: {"likes": 653, "downloads": 45260, "num_parameters": 117561977600, "pipeline_tag": "text-generation"}

### 원문 설명
likes=653, downloads=45260, pipeline_tag=text-generation
...
### 요약
Upstage에서 공개한 250B 규모의 대형 언어 모델인 upstage/Solar-Open2-250B가 Hugging Face에서 주목받고 있습니다. 해당 모델은 text-generation 태스크를 위한 모델로, 높은 다운로드 수를 기록하며 트렌드에 진입했습니다.

### 핵심 포인트
- upstage/Solar-Open2-250B 모델의 Hugging Face 트렌드 진입
- 약 250B 규모의 파라미터를 보유한 text-generation 모델
- 2,784회의 downloads와 557개의 likes를 기록하며 사용자 관심을 확보

**태그**: Upstage, Solar-Open2-250B, text-generation

**Metrics**: {"likes": 557, "downloads": 2784, "num_parameters": 250287810304, "pipeline_tag": "text-generation"}

### 원문 설명
likes=557, downloads=2784, pipeline_tag=text-generation
