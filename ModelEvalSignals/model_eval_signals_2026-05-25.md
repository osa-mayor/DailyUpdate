...
### 오늘의 핵심 포인트
- ByteDance의 any-to-any 모델 'Lance' 및 Cohere의 이미지-텍스트 통합 모델 등 멀티모달 모델 트렌드 지속
- SWE-bench의 새로운 평가 방식(CodeClash) 및 에이전트 학습 환경(SWE-smith) 등 소프트웨어 엔지니어링 에이전트 고도화
- LiveCodeBench를 통한 Gemini 및 O4-Mini 등 최신 모델들의 코딩 성능 경쟁 가속화
...
- pipeline_tag는 any-to-any로 분류되어 다양한 모달리티 간 작업이 가능함을 시사합니다.
- 현재 753개의 likes와 1,474개의 downloads를 기록하며 사용자 관심을 끌고 있습니다.

**태그**: ByteDance, Lance, any-to-any

**Metrics**: {"likes": 753, "downloads": 1474, "num_parameters": 0, "pipeline_tag": "any-to-any"}

### 원문 설명
likes=753, downloads=1474, pipeline_tag=any-to-any
...
- 5,627회의 다운로드와 189개의 likes를 기록하며 트렌딩 중

**태그**: CohereLabs, image-text-to-text, LLM

**Metrics**: {"likes": 189, "downloads": 5627, "num_parameters": 125789176144, "pipeline_tag": "image-text-to-text"}

### 원문 설명
likes=189, downloads=5627, pipeline_tag=image-text-to-text
...
### 요약
Tencent에서 공개한 Hy-MT2-30B-A3B 모델이 Hugging Face에서 주목받고 있습니다. 이 모델은 약 30B 규모의 파라미터를 가진 번역 특화 모델입니다.

### 핵심 포인트
- Tencent에서 공개한 30B 규모의 모델
- 주요 태스크는 translation(번역) 작업
- likes 306회, downloads 1,243회를 기록하며 트렌딩 중

**태그**: Tencent, Hy-MT2-30B-A3B, translation

**Metrics**: {"likes": 306, "downloads": 1243, "num_parameters": 30064725888, "pipeline_tag": "translation"}

### 원문 설명
likes=306, downloads=1243, pipeline_tag=translation
